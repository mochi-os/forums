...
def database_upgrade(to_version):
    ...
    if to_version == 30:
        ...
        mochi.db.execute("create index if not exists score_cache_forum on score_cache(forum, computed)")
    if to_version == 31:
        # Catch-up migration: legacy migrations 1-25 were deleted, leaving
        # dormant per-entity forum DBs at user_version 25 missing many columns
        # and tables. A backup-restore-and-reboot triggers the lazy migration
        # path on those DBs for the first time, exposing every gap at once.
        # All checks are idempotent so freshly-created DBs (which already have
        # everything from database_create()) are unaffected.

        # forums: legacy columns
        cols = [r["name"] for r in mochi.db.table("forums")]
        ...
        # comments: status, remover, reason
        cols = [r["name"] for r in mochi.db.table("comments")]
        if "status" not in cols:
        ...
        # Tables added by deleted legacy migrations (moderation, restrictions,
        # reports, rss, tags). Mirror database_create() exactly.
        mochi.db.execute("""create table if not exists moderation (
            id text primary key, forum text not null, moderator text not null, action text not null,
            type text not null, target text not null, author text, reason text not null default '',
            created integer not null )""")
        ...
        mochi.db.execute("create index if not exists tags_qid on tags( qid )")
    if to_version == 31:
        # Some forum DBs still carry the original settings(name, value) table
        # from schema v1: v11 dropped it but was later removed in the legacy
        # migration cleanup, so DBs that never ran v11 kept the old shape.
        # v29's "create table if not exists" was then a no-op on those DBs,
        # and the follow-up insert into settings(id, sort) failed. Detect the
        # stale shape by absence of the id column and rebuild.
        cols = [r["name"] for r in mochi.db.table("settings")]
        if "id" not in cols:
            mochi.db.execute("drop table if exists settings")
            mochi.db.execute("create table settings ( id integer primary key check ( id = 1 ), sort text not null default '' )")
            mochi.db.execute("insert into settings ( id, sort ) values ( 1, '' )")
