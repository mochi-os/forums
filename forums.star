...
def action_view(a):
    ...
    if forum_id:
        ...
        # List all forums
        forums = mochi.db.query("select * from forums order by updated desc")
        posts = mochi.db.query("select * from posts order by updated desc")
...
def action_members_edit(a):
    ...
    members = mochi.db.query("select * from members where forum=? order by name", forum["id"])
...
def action_unsubscribe(a):
    ...
    mochi.db.query("delete from votes where forum=?", forum["id"])
    mochi.db.query("delete from comments where forum=?", forum["id"])
    mochi.db.query("delete from posts where forum=?", forum["id"])
    mochi.db.query("delete from members where forum=?", forum["id"])
    mochi.db.query("delete from forums where id=?", forum["id"])
...
def action_post_view(a):
    ...
    def get_comments(parent_id, depth):
        ...
        comments = mochi.db.query("select * from comments where forum=? and post=? and parent=? order by created desc",
...
def action_comment_create(a):
    ...
    mochi.db.query("replace into comments ( id, forum, post, parent, member, name, body, created ) values ( ?, ?, ?, ?, ?, ?, ?, ? )",
        id, forum["id"], post_id, parent_id or "", a.user.identity.id, a.user.identity.name, body, now)
    
    mochi.db.query("update posts set updated=?, comments=comments+1 where id=?", now, post_id)
    mochi.db.query("update forums set updated=? where id=?", now, forum["id"])
...
def action_post_vote(a):
    ...
    if old_vote:
        if old_vote["vote"] == "up":
            mochi.db.query("update posts set up=up-1 where id=?", post["id"])
        elif old_vote["vote"] == "down":
            mochi.db.query("update posts set down=down-1 where id=?", post["id"])
    
    # Add new vote
    mochi.db.query("replace into votes ( forum, post, comment, voter, vote ) values ( ?, ?, '', ?, ? )",
    ...
    now = mochi.time.now()
    mochi.db.query("update posts set updated=? where id=?", now, post["id"])
    mochi.db.query("update forums set updated=? where id=?", now, forum["id"])
...
def action_comment_vote(a):
    ...
    if old_vote:
        if old_vote["vote"] == "up":
            mochi.db.query("update comments set up=up-1 where id=?", comment["id"])
        elif old_vote["vote"] == "down":
            mochi.db.query("update comments set down=down-1 where id=?", comment["id"])
...
def event_comment_create_event(e):
    ...
    mochi.db.query("update posts set updated=?, comments=comments+1 where id=?", created, post)
...
def event_comment_submit_event(e):
    ...
    mochi.db.query("update posts set updated=?, comments=comments+1 where id=?", now, post)
    mochi.db.query("update forums set updated=? where id=?", now, forum["id"])
...
def event_comment_update_event(e):
    ...
    now = mochi.time.now()
    mochi.db.query("update comments set up=?, down=? where id=?", comment.get("up", 0), comment.get("down", 0), id)
    mochi.db.query("update posts set updated=? where id=?", now, old_comment["post"])
    mochi.db.query("update forums set updated=? where id=?", now, old_comment["forum"])
...
def event_comment_vote_event(e):
    ...
    if old_vote:
        if old_vote["vote"] == "up":
            mochi.db.query("update comments set up=up-1 where id=?", comment_id)
        elif old_vote["vote"] == "down":
            mochi.db.query("update comments set down=down-1 where id=?", comment_id)
...
def event_member_update_event(e):
    ...
    mochi.db.query("update members set role=? where forum=? and id=?", role, forum["id"], e.content("to"))
...
def event_post_update_event(e):
    ...
    now = mochi.time.now()
    mochi.db.query("update posts set up=?, down=? where id=?", post.get("up", 0), post.get("down", 0), id)
    mochi.db.query("update posts set updated=? where id=?", now, id)
    mochi.db.query("update forums set updated=? where id=?", now, old_post["forum"])
...
def event_post_vote_event(e):
    ...
    if old_vote:
        if old_vote["vote"] == "up":
            mochi.db.query("update posts set up=up-1 where id=?", post_id)
        elif old_vote["vote"] == "down":
            mochi.db.query("update posts set down=down-1 where id=?", post_id)
...
def event_unsubscribe_event(e):
    ...
    mochi.db.query("delete from members where forum=? and id=?", forum["id"], member_id)
